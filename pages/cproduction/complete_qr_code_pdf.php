<?php
       ...
       while ($row = mysqli_fetch_assoc($size_list)){
         $all_size[] = $row;
          ...
          //                       print_r($row);
          // }
          $cProductionResult = $selectCProduction->select_with_id_qr($id);
                    //
                    // print '<pre/>';
                    // print_r($lineName);
                    //exit();
          ...
          while ($row = mysqli_fetch_assoc($cProductionResult))
          {
             ...
             while ($row_c = mysqli_fetch_assoc($cProductionChild)){
              ...
              $getOrderQtyByPO = $selectCProduction->select_order_qty_by_po($row['po']);
               ...
               $page_number++;
               $childLineNameInfo = "";
               $childLineName = $selectCProduction->select_line_name_by_child_id_qr($row_c['id']);
               ...
               $country_codes = explode(",",$row_c['country_code']);
               $ticket_size = "";
               foreach ($all_size as $row_size)
               {
                  if($row_c['ticket_size'] == $row_size['id']){
               ...
               QRcode::png($qrdata, $filename, $errorCorrectionLevel,
                $matrixPointSize,2);
                          ...
                          if($getOrderQty['total_quantity']){
                            if($row_c['country_name'] == "CN"){
                            $pdf->SetFont('Arial','B',10.5);
                            $pdf->Text(16.5,43+$linesSpace,$row_c['country_name']);
                            $pdf->SetFont('Arial','B',7);
                            $pdf->Text(22,43+$linesSpace,"=".$getOrderQty['total_quantity']."(".$orderQtyByPO['total_quantity'].")");
                            }else {
