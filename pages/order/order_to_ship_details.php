    ...
    <div class="w3-animate-right">
        ...
        <div class="box-default widget-body">
            <div class="box-header with-border " style="background: linear-gradient(to bottom, #0000ff 0%, #cc6600 100%);padding: 8px;">
                ...
                <div class="row">
                  <a class="btn badge-text badge-text-small info" href="pages/order/update.php?id=<?php echo $order_details['id']; ?>" style="margin-left:800px;float:left">Update</a>
                  <a class="btn badge-text badge-text-small info" href="pages/order/order_to_ship_excel.php?id=<?php echo $order_details['id']; ?>" style="margin-left:10px;">Excel</a>
                </div>
                     ...
                     $size_id_list=0;
                      ...
                      foreach ($orderedCountryList as $country) {
                       ...
                       <table class="w3-table-all w3-small" border="1">
                          ...
                          <tr>
                             <td style="width:140px;"></td>
                             <td style="width:100px;" class="text-center btn-gradient-01">Cutting Production Qty
                             </td>
                             ...
                             $grandTotal = 0;
                               foreach ($size_id as  $size_info)
                               {
                                  ...
                                  foreach ($cut_pro_info as  $row)
                                  {
                                    ...
                                    // $cut_pro_ticket_info = $selectCProduction->select_ticket_no_by_size_cut_pro_id($row['id'],$size_info['id']);
                                    // foreach ($cut_pro_ticket_info as  $cut_pro_ticket)
                                    // {
                                    //    if($cut_pro_ticket['ticket_no']){
                                    //      $cut_pro_qty_by_size_country = $selectCProduction->select_qty_by_size_cut_pro_id($row['id'],$cut_pro_ticket['ticket_no'],$country['country_id']);
                                    //      foreach ($cut_pro_qty_by_size_country as  $cut_pro_qty_by_size_country_info)
                                    //      {
                                    //        $total_size_quantity += $cut_pro_qty_by_size_country_info['total_quantity'];

                                    //      }
                                    //    }
                                    // }
                                  }
                                  $grandTotal += $total_size_quantity;
                                  ?>
                                  <td style="color:black;width:20px;"><?php echo $total_size_quantity; ?></td>
                                  <?php
                               }
                             ...
                             ?>
                              ...
                              <td class="text-center" style="color:black;"><?php echo $grandTotal; ?></td>
                           </tr>
                           ...
                           $iissueTotal = 0;
                             foreach ($size_id as  $size_info)
                             {
                                ...
                                foreach ($cut_pro_info2 as  $row)
                                {
                                  ...
                                  // print_r($row);
                                  // echo  '</pre>';
                                  $total_iissue_quantity_size += $row['total_scan_quantity'];
                                }
                                $iissueTotal += $total_iissue_quantity_size;


                           //$iissue_scan_qty = $ob->select_iissue_scan_qty2($info['po']);
                      		   // $order_child_quantity_country = $selectInputIssue->select_child_with_order_and_country($order_id,$country['country_id']);
                      		   // $total = 0;
                      		   // while ($row = mysqli_fetch_assoc($order_child_quantity_country))
                      		   // {
                      	   ?>
                      		   <td style="color:black;width:20px;"><?php echo $total_iissue_quantity_size; ?></td>
                      	   <?php
                      			 //$total += $quantity;
                      		   }
                      	   ?>

                            <td class="text-center" style="color:black;"><?php echo $iissueTotal; ?></td>
                           </tr>
